...
public class curve_route : MonoBehaviour
{
    ...
    private Vector3 node_pos;

    public Vector3 Getpos(float t) 
    {
        node_pos = Mathf.Pow(1 - t, 3) * controlPoints[0].position + 3 * Mathf.Pow(1 - t, 2) * t * controlPoints[1].position + 3 * (1 - t) * Mathf.Pow(t, 2) * controlPoints[2].position + Mathf.Pow(t, 3) * controlPoints[3].position;
    ...
    private void OnDrawGizmos()
    {
        for (float t = 0; t <= 1; t += 0.025f)
        {
            ...
            Gizmos.DrawSphere(node_pos, 0.25f);
        }
